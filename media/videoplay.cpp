...
void VideoPlay::start()
{
    ...
    for (unsigned int i = 0; i < infmt_ctx->nb_streams; i++) {
        if (infmt_ctx->streams[i]->codec->codec_type == AVMEDIA_TYPE_VIDEO && video_stream_idx == -1) {
            video_stream_idx = i;
        }
        else if (infmt_ctx->streams[i]->codec->codec_type == AVMEDIA_TYPE_AUDIO && audio_stream_idx == -1) {
            audio_stream_idx = i;
        }
    }
    ...
    if (thread_audio) {
        ...
        if (stopped) {
            thread_audio->wait(3000);
            thread_video->wait(3000);
        } else {
            thread_audio->wait();
            thread_video->wait();
        }
...
void VideoPlay::stop()
{
    ...
    if (audio_queue) {
        audio_queue->queue_abort();
    }
    if (video_queue) {
        video_queue->queue_abort();
    }
    if (play_audio) {
        play_audio->stop();
    }
...
void VideoPlay::force_close()
{
    stop();  // 先执行正常停止
    
    /* 强制关闭格式上下文，唤醒 av_read_frame() */
    if (infmt_ctx) {
        avformat_close_input(&infmt_ctx);
        infmt_ctx = nullptr;
    }
...
void VideoPlay::slot_ready_frame(QImage image)
{
    emit sig_ready_frame(image);
}
