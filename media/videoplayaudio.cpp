...
void VideoPlayAudio::start()
{
    ...
    /* 初始化、打开音频流解码器 */
    audio_dec_ctx = infmt_ctx->streams[audio_stream_idx]->codec;
    if (!audio_dec_ctx) {
        qWarning() << "can not get audio codec context" << endl;
        return;
    }
    AVCodec *audio_codec = avcodec_find_decoder(audio_dec_ctx->codec_id);
    if (!audio_codec) {
    ...
    while (1) {
        ...
        /* 处理音频数据包 */
        pkt = av_packet_alloc();
        int pop_ret = audio_queue->queue_pop(pkt);
        if (!pop_ret) {
            break;
        }
        if (pop_ret) {    // 从音频队列中获取数据包
            ...
            /* 旧版FFmpeg解码API */
            int got_frame = 0;
            ret = avcodec_decode_audio4(audio_dec_ctx, pcm_frame_src, &got_frame, pkt);
            if (ret < 0) {
                qWarning() << "audio: avcodec_decode_audio4 error" << endl;
                break;
            }
            if (got_frame) {
                /* 更新音频时钟, 发送信号 */
