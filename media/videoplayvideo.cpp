...
void VideoPlayVideo::start()
{
    ...
    /* 初始化、打开视频流解码器 */
    video_dec_ctx = infmt_ctx->streams[video_stream_idx]->codec;
    if (!video_dec_ctx) {
        qWarning() << "can not get video codec context" << endl;
        return;
    }
    AVCodec *video_codec = avcodec_find_decoder(video_dec_ctx->codec_id);
    if (!video_codec) {
    ...
    rgb_frame_tar->linesize[0] = 200 * 3;       // 手动设置 linesize: width * 3(RGB)
    ret = av_frame_get_buffer(rgb_frame_tar, 32);        // 分配内存缓冲区（32字节对齐）
    if (ret < 0) {
    ...
    qDebug() << "Video decoder started:" << video_dec_ctx->width << "x" << video_dec_ctx->height;
    ...
    while (1) {
        ...
        /* 处理视频数据包 */
        pkt = av_packet_alloc();
        int pop_ret = video_queue->queue_pop(pkt);
        if (!pop_ret) {
            break;
        }
        if (pop_ret) {    // 从视频队列中取数据包
            ...
            /* 旧版FFmpeg解码API */
            int got_picture = 0;
            ret = avcodec_decode_video2(video_dec_ctx, rgb_frame_src, &got_picture, pkt);
            if (ret < 0) {
                qWarning() << "video: avcodec_decode_video2 error" << endl;
                break;
            }
            if (got_picture) {
                /* 缩放视频帧 */
                ...
                /* 创建 QImage 副本（必须深拷贝，因为原始数据会被覆盖） */
                QImage frameImage(
                    rgb_frame_tar->data[0],
                    rgb_frame_tar->width,
                    rgb_frame_tar->height,
                    rgb_frame_tar->linesize[0],
                    QImage::Format_RGB888
                );

                emit sig_frame_ready(frameImage.copy());  // 必须复制数据
            }
            av_packet_unref(pkt);
        }
    }
