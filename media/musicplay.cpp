    ...
    if (!volume_elem) {
        ...
        while (elem) {
            ...
            if (snd_mixer_selem_has_playback_volume(elem)) {
                if (!volume_elem) {
                    volume_elem = elem;
                    qDebug() << "Found volume control:" << elem_name << endl;
                }
            }
        ...
        if (!volume_elem) {
            qWarning() << "No playback volume element found!" << endl;
        }
    }
}
...
void MusicPlay::start()
{
    ...
    /* 获取流的编解码器上下文 (旧版FFmpeg API) */
    AVCodecContext *stream_codec_ctx = infmt_ctx->streams[audio_stream_idx]->codec;
    /* 获取解码器 */
    AVCodec* decodec = avcodec_find_decoder(stream_codec_ctx->codec_id);
    if (!decodec) {
        qWarning() << "avcodec_find_decoder error" << endl;
        avformat_close_input(&infmt_ctx);
        return;
    }
    /* 创建并复制解码器上下文 (旧版FFmpeg API: 使用avcodec_copy_context) */
    ...
    ret = avcodec_copy_context(decodec_ctx, stream_codec_ctx);
    if (ret < 0) {
        qWarning() << "avcodec_copy_context error" << endl;
    ...
    while (1) {
        ...
        /* 检查是否需要跳转 */
        {
            ...
            if (need_seek) {
                ...
                int64_t target_ts = static_cast<int64_t>(seek_position / av_q2d(infmt_ctx->streams[audio_stream_idx]->time_base));

                ret = av_seek_frame(infmt_ctx, audio_stream_idx, target_ts, AVSEEK_FLAG_BACKWARD);
                if (ret < 0) {
                ...
                locker.relock();
                played_samples = seek_position * rate;
                curr_time = seek_position;
                need_seek = false;
            }
        }
        /* 读取一个数据包 */
        ...
        if (input_packet->stream_index == audio_stream_idx) {
            int got_frame = 0;
            uint8_t *write_2_pcm = nullptr;

            // 一个 packet 可能包含多帧，用 while 循环解码直到数据消耗完
            while (input_packet->size > 0) {
                ret = avcodec_decode_audio4(decodec_ctx, pframeSRC, &got_frame, input_packet);
                if (ret < 0) {
                    qWarning() << "avcodec_decode_audio4 error" << endl;
                    break;
                }
                if (got_frame) {
                    // 重采样
                    int converted_data_num = swr_convert(pcm_convert_ctx,
                                                         pframePCM->data,
                                                         pframePCM->nb_samples,
                                                         (const uint8_t **)pframeSRC->extended_data,
                                                         pframeSRC->nb_samples);

                    played_samples += converted_data_num;
                    curr_time = played_samples / rate;
                    emit send_curr_time(curr_time);

                    write_2_pcm = pframePCM->data[0];
                    snd_pcm_writei(pcm, write_2_pcm, converted_data_num);
                }
                // 注意：ret 表示已使用的字节数，需要更新 input_packet 的数据指针和剩余大小
